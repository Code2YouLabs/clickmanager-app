...
describe('Preparação persistente', () => {
  ...
  it('mantém o catálogo em processamento após o tempo visual se o backend ainda não concluiu', fakeAsync(() => {
    TestBed.configureTestingModule({imports:[SetupProgressComponent,NoopAnimationsModule]});
    const fixture = TestBed.createComponent(SetupProgressComponent);
    fixture.componentRef.setInput('job', job);
    fixture.componentRef.setInput('onboarding', true);
    fixture.componentRef.setInput('animarEtapas', true);
    const concluido = jasmine.createSpy('concluido');
    fixture.componentInstance.etapasConcluidas.subscribe(concluido);
    fixture.detectChanges();
    expect(fixture.componentInstance.tarefas[0].state).toBe('PROCESSANDO');
    tick(1999); fixture.detectChanges();
    expect(fixture.componentInstance.tarefas[0].state).toBe('PROCESSANDO');
    tick(2001); fixture.detectChanges();
    expect(fixture.componentInstance.tarefas[2].state).toBe('PROCESSANDO');
    tick(2000); fixture.detectChanges();
    expect(fixture.componentInstance.tarefas[2].state).toBe('PROCESSANDO');
    expect(concluido).not.toHaveBeenCalled();
    fixture.componentRef.setInput('job', {...job,status:'CONCLUIDO',fase:'CONCLUIDO',processados:150,smartCalcPreparados:2});
    fixture.detectChanges();
    expect(fixture.componentInstance.tarefas[3].state).toBe('PROCESSANDO');
    tick(4000); fixture.detectChanges();
    expect(concluido).toHaveBeenCalledTimes(1);
    expect(fixture.componentInstance.titulo).toBe('Tudo pronto para você!');
    fixture.destroy();
  }));
});
